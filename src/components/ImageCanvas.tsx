...
const LIGHTING_SETTINGS: Record<string, LightingPreset> = {
  ...
  afternoon: { r: 1, g: 1, b: 1, brightness: 1.15 },
  evening: { r: 1.15, g: 0.95, b: 0.85, brightness: 0.9 },
  night: { r: 1.2, g: 1, b: 0.8, brightness: 0.5 },
  cloudy: { r: 0.95, g: 1, b: 1.1, brightness: 0.9 }
};
...
export default function ImageCanvas({ imageUrl, selectedColor, whiteBalance, lighting }: ImageCanvasProps) {
  ...
  useEffect(() => {
    ...
    process();
  }, [rawImageData, whiteBalance, sam, lighting]);
  ...
  const recolorWall = (wall: WallSurface, newColor: string) => {
    ...
    if (canvas) {
      const ctx = canvas.getContext('2d');
      if (ctx) applyLighting(ctx, canvas.width, canvas.height, lighting);
    }
    saveToHistory();
  };
